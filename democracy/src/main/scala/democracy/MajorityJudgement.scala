...
case class Election(description: String, candidates: Set[Candidate]):
  ...
  def findWinner(gradesPerCandidate: Map[Candidate, Seq[Grade]]): Candidate =
    ...
    if gradesPerCandidate.forall((candidate, grades) => grades.isEmpty) then
      ...
      // median grade.
      val bestMedianGrade: Grade = gradesPerCandidate.values.filter(_.nonEmpty).map(seq => Grade.median(seq)).maxBy(_.ordinal)
      ...
      val bestCandidates: Map[Candidate, Seq[Grade]] = gradesPerCandidate.filter(
        (candidate, grades) => Grade.median(grades) == bestMedianGrade
      )
