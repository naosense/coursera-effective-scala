...
final class Wikigraph(client: Wikipedia):
  ...
  def namedLinks(of: ArticleId): WikiResult[Set[String]] =
    client.linksFrom(of).flatMap { ids =>
      WikiResult.traverse(ids.toSeq)(client.nameOfArticle).map(_.toSet)
    }
    ...
    def iter(visited: Set[ArticleId], q: Queue[(Int, ArticleId)]): WikiResult[Option[Int]] =
      if q.isEmpty then
        ...
        if depth >= maxDepth then
          ...
          client.linksFrom(article)
            .flatMap(ids => {
              if ids.contains(target) then
                WikiResult.successful(Some(depth))
              else
                iter(visited + article, remaining.enqueueAll(ids.diff(visited).map(id => (depth + 1, id))))
            })
  ...
  def distanceMatrix(titles: List[String], maxDepth: Int = 50): WikiResult[Seq[(String, String, Option[Int])]] =
    ...
    WikiResult.traverse(allArticlePairs) { (start, target) =>
      client.searchId(start).zip(client.searchId(target)).flatMap { (sid, tid) =>
        breadthFirstSearch(sid, tid, maxDepth).flatMap { distance => WikiResult.successful(start, target, distance) }
      }
    }
