...
trait HeapProperties(val heapInterface: HeapInterface):
  ...
  val meldingSmallHeaps: (String, Prop) =
    ...
    forAll { (x: Int, y: Int) =>
      ...
      // highest value
      val deleteTwoMinAndFindMin: Boolean = findMin(deleteMin(deleteMin(meldedHeap))) == hi
      // check that inserting the lowest value to the melded heap, and then
      // finding the minimal element returns the lowest value
      val insertMinAndFindMin: Boolean = findMin(insert(lo, insert(lo, meldedHeap))) == lo
  ...
  val meldingHeaps: (String, Prop) =
    def check(meldHeap: List[Node], heap1: List[Node], heap2: List[Node]): Boolean =
      if isEmpty(heap1) && isEmpty(heap2) then
        isEmpty(meldHeap)
      else if isEmpty(heap1) then
        if findMin(meldHeap) == findMin(heap2) then
          check(deleteMin(meldHeap), heap1, deleteMin(heap2))
        else
          false
      else if isEmpty(heap2) then
        if findMin(meldHeap) == findMin(heap1) then
          check(deleteMin(meldHeap), deleteMin(heap1), heap2)
        else
          false
      else
        if findMin(meldHeap) == findMin(heap1) then
          check(deleteMin(meldHeap), deleteMin(heap1), heap2)
        else if findMin(meldHeap) == findMin(heap2) then
          check(deleteMin(meldHeap), heap1, deleteMin(heap2))
        else
          false
