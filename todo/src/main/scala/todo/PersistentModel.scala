...
object PersistentModel extends Model:
  ...
  private val idStore: mutable.LinkedHashMap[Id, Task] =
    mutable.LinkedHashMap.from(loadTasks().tasks)
  ...
  def update(id: Id)(f: Task => Task): Option[Task] =
    val res = idStore.updateWith(id)(opt => opt.map(f))
    saveTasks(tasks)
    res

  def delete(id: Id): Boolean =
    val res = idStore.remove(id).isDefined
    saveTasks(tasks)
    res
  ...
  def complete(id: Id): Option[Task] =
    val res = update(id)(opt => opt.complete)
    saveTasks(tasks)
    res
