...
trait DecoderInstances:
  ...
  given listDecoder[A](using decoder: Decoder[A]): Decoder[List[A]] =
    ...
    def decodeAndPrepend(item: Json, previouslyDecodedItems: List[A]): Option[List[A]] =
      decoder.decode(item).map(a => a :: previouslyDecodedItems)
    ...
    def processItem(item: Json, maybePreviouslyDecodedItems: Option[List[A]]): Option[List[A]] =
      maybePreviouslyDecodedItems.flatMap(list => decodeAndPrepend(item, list))
...
@main def run(): Unit =
  ...
  val personStr = renderJson(Person("Harry", 21))
  println(parseJson(personStr).flatMap(_.decodeAs[Person]))
